"""office_edit_word_script — Builder JS edit on Word source."""
...
from aiecs.tools.office_tool.word.schemas.section_spec import WordEditScriptArgs

logger = logging.getLogger(__name__)

TOOL_NAME = "office_edit_word_script"

TOOL_DEF = {
    "name": TOOL_NAME,
    "description": (
        f"[Word] Edit Word document with raw Builder JS (edit_script only — OpenFile/SaveFile injected). "
...
async def office_edit_word_script(
    edit_script: str,
    output_path: str,
    ...
    **kwargs: Any,
    ...
    try:
        args = WordEditScriptArgs.model_validate(
            {
    ...
    return await run_builder_on_source(
        fetch_url, file_ext, args.edit_script, args.output_path, client=client
    )


handler = office_edit_word_script

__all__ = ["TOOL_NAME", "TOOL_DEF", "handler", "office_edit_word_script"]
