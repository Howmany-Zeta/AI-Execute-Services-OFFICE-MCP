"""office_merge_word — merge Word documents."""
...
from aiecs.tools.office_tool.word.schemas.section_spec import WordMergeArgs

logger = logging.getLogger(__name__)

TOOL_NAME = "office_merge_word"

TOOL_DEF = {
    "name": TOOL_NAME,
    "description": (
        f"[Word] Merge Word documents into one file. source_paths ({ACCEPTED_SOURCE_PATH_FORMATS}) "
        "or source_urls. Options: add_page_break, add_toc."
    ),
    "inputSchema": {
        "type": "object",
        "properties": {
...
async def office_merge_word(
    ...
    **kwargs: Any,
    ...
    try:
        args = WordMergeArgs.model_validate(
            {
    ...
    script = build_merge_script(
        ...
        add_page_break=args.options.add_page_break,
        add_toc=args.options.add_toc,
    )
    return await run_builder_script(script, output_path=args.output_path, client=client)


handler = office_merge_word

__all__ = ["TOOL_NAME", "TOOL_DEF", "handler", "office_merge_word"]
