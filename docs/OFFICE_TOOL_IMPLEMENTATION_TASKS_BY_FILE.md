...
| MCP 工具 | **6**（adapter 硬编码 `OFFICE_TOOLS`） |
| `office_tool_adapter.py` | 手工 import 六工具 |
| `main_mcp.py` health | `tool_count = len(list_tools())`，**无** `canonical_count` |
| `tests/office_mcp/` | **扁平** `test_office_*.py`；无 `{core,word,...}/` 子目录 |
| `pyproject.toml` markers | **无** `word` / `presentation` / `spreadsheet` / `pdf`（**`--strict-markers` 已启用**） |
| 垂直实现设计文档 | **已有** 4 份 `OFFICE_MCP_*_IMPLEMENTATION_DESIGN.md` |
...
### [ ] **Task OT-013** — `aiecs/tools/office_tool/core/__init__.py`（新建）

| 字段 | 内容 |
|------|------|
| **必须完成** | 包初始化；导出 `builder_runtime`、`builder_js` public API |

### [ ] **Task OT-014** — `aiecs/tools/office_tool/core/builder_js.py`（新建）
...
### [ ] **Task OT-015** — `aiecs/tools/office_tool/core/builder_runtime.py`（新建）
...
### [ ] **Task OT-016** — `aiecs/tools/office_tool/edit_document.py`（改）
...
### [ ] **Task OT-017** — `aiecs/tools/office_tool/merge_document.py`（改）

| 字段 | 内容 |
|------|------|
| **必须完成** | 脚本生成保留；执行改 `run_builder_script` |

### [ ] **Task OT-018** — `aiecs/tools/office_tool/apply_template.py`（改）

| 字段 | 内容 |
|------|------|
| **必须完成** | 执行改 `run_builder_script` |

### [ ] **Task OT-019** — `aiecs/tools/office_tool/execute_builder.py`（改）

| 字段 | 内容 |
|------|------|
| **必须完成** | 执行改 `run_builder_script` |

### [ ] **Task OT-020** — `tests/office_mcp/core/test_builder_runtime.py`（新建）
...
### [ ] **Task OT-021** — 回归：`tests/office_mcp/test_office_*.py`
...
### [ ] **Task OT-022** — `aiecs/tools/office_tool/read_document.py` / `call_api.py`
...
### [ ] **Task OT-023** — `aiecs/tools/office_tool/core/categories.py`（新建）
...
### [ ] **Task OT-024** — `aiecs/tools/office_tool/conversion_output.py`（改 shim）
...
### [ ] **Task OT-025** — `aiecs/tools/office_tool/core/errors.py`（新建）

| 字段 | 内容 |
|------|------|
| **必须完成** | `err(text)`, `ok(**kwargs)`（ADR-006） |

### [ ] **Task OT-026** — `aiecs/tools/office_tool/core/read_response.py`（新建）
...
### [ ] **Task OT-027** — `aiecs/tools/office_tool/core/builder_json_sidecar.py`（新建）

| 字段 | 内容 |
|------|------|
| **必须完成** | `SIDECAR_FILENAME`, `build_sidecar_extract_script`, `read_sidecar_json`（§4.4） |

### [ ] **Task OT-028** — `aiecs/tools/office_tool/core/coarse_read.py`（新建）
...
### [ ] **Task OT-029** — `aiecs/tools/office_tool/core/source.py`（新建）

| 字段 | 内容 |
|------|------|
| **必须完成** | 自 `source_resolver.py` 移动；保留 `resolve_document_source` |

### [ ] **Task OT-030** — `aiecs/tools/office_tool/source_resolver.py`（改 shim）

| 字段 | 内容 |
|------|------|
| **必须完成** | `from core.source import *` |

### [ ] **Task OT-031** — `aiecs/tools/office_tool/core/storage/paths.py`（新建）

| 字段 | 内容 |
|------|------|
| **必须完成** | 自 `storage_paths.py` 移动 |

### [ ] **Task OT-032** — `aiecs/tools/office_tool/core/storage/backend.py`（新建）

| 字段 | 内容 |
|------|------|
| **必须完成** | 自 `storage.py` 移动：`upload_to_storage`, `copy_storage_file`, `get_file_ext`, 等 |

### [ ] **Task OT-033** — `aiecs/tools/office_tool/core/storage/object_fetch.py`（新建）

| 字段 | 内容 |
|------|------|
| **必须完成** | 自 `object_fetch.py` 移动 |

### [ ] **Task OT-034** — `aiecs/tools/office_tool/core/storage/__init__.py`（新建）

| 字段 | 内容 |
|------|------|
| **必须完成** | 聚合 storage public API |

### [ ] **Task OT-035** — `aiecs/tools/office_tool/storage.py` / `storage_paths.py` / `object_fetch.py`（改 shim）

| 字段 | 内容 |
|------|------|
| **必须完成** | re-export `core.storage.*` |

### [ ] **Task OT-036** — `aiecs/tools/office_tool/core/docbuilder_script.py`（新建）

| 字段 | 内容 |
|------|------|
| **必须完成** | 自 `docbuilder_script.py` 移动：`script_to_url`, `get_script` |

### [ ] **Task OT-037** — `aiecs/tools/office_tool/docbuilder_script.py`（改 shim）

| 字段 | 内容 |
|------|------|
| **必须完成** | re-export |

### [ ] **Task OT-038** — `aiecs/tools/office_tool/legacy/read_document.py`（新建）
...
### [ ] **Task OT-039** — `aiecs/tools/office_tool/read_document.py`（改 shim）

| 字段 | 内容 |
|------|------|
| **必须完成** | re-export `legacy.read_document` 或 thin forward |

### [ ] **Task OT-040** — `tests/office_mcp/core/test_categories.py`（新建）

| 字段 | 内容 |
|------|------|
| **必须完成** | docx/pptx/xlsx/pdf/unknown 分类 |

### [ ] **Task OT-041** — `tests/office_mcp/core/test_read_response.py`（新建）

| 字段 | 内容 |
|------|------|
| **必须完成** | 四类 category alias mirror |

### [ ] **Task OT-042** — `tests/office_mcp/core/test_storage.py`（新建）

| 字段 | 内容 |
|------|------|
| **必须完成** | 自 `test_office_storage*.py` 迁路径或 re-import smoke |

### [ ] **Task OT-043** — `tests/office_mcp/test_office_read_document*.py`（改）

| 字段 | 内容 |
|------|------|
| **必须完成** | import 路径更新；**断言不变** |

### [ ] **Task OT-044** — `aiecs/main_mcp.py`（M1 可选）
...
### [ ] **Task OT-045b** — `tests/office_mcp/conftest.py` + `probe_ds_capabilities.py` 骨架（M1/M3）
...
### [ ] **Task OT-045c** — `pyproject.toml`（M1 · `word` marker）
...
### [ ] **Task OT-046** — `aiecs/tools/office_tool/word/__init__.py`（新建）

### [ ] **Task OT-047** — `word/parser/html.py`（新建，W0）
...
### [ ] **Task OT-048** — `word/parser/document.py`（新建，W1）
...
### [ ] **Task OT-049** — `word/schemas/read.py` / `section_spec.py` / `edit_ops.py`（新建，W2）
...
### [ ] **Task OT-050** — `word/builder/create.py` / `edit.py` / `merge.py` / `template.py`（新建）
...
### [ ] **Task OT-051** — `word/tools/read.py`（新建，W1）
...
### [ ] **Task OT-052** — `word/tools/create.py`（新建，W2）

| 字段 | 内容 |
|------|------|
| **必须完成** | `office_create_word` → `run_builder_script` |

### [ ] **Task OT-053** — `word/tools/edit.py`（新建，W2）

| 字段 | 内容 |
|------|------|
| **必须完成** | `office_edit_word` → `run_builder_on_source` |

### [ ] **Task OT-054** — `word/tools/merge.py`（新建，W3）

| 字段 | 内容 |
|------|------|
| **必须完成** | `office_merge_word` |

### [ ] **Task OT-055** — `word/tools/template.py`（新建，W3）

| 字段 | 内容 |
|------|------|
| **必须完成** | `office_apply_template_word` |

### [ ] **Task OT-056** — `word/tools/edit_script.py`（新建，W3）

| 字段 | 内容 |
|------|------|
| **必须完成** | 自 `edit_document.py` 迁入；`office_edit_word_script` |

### [ ] **Task OT-057** — `legacy/edit_document.py` / `merge_documents.py` / `apply_template.py`（新建，W3）
...
### [ ] **Task OT-058** — 根 `merge_document.py` / `apply_template.py` / `edit_document.py`（W3 shim）

| 字段 | 内容 |
|------|------|
| **必须完成** | re-export 或删逻辑留 shim（ADR-022：M7 保留） |

### [ ] **Task OT-059** — `tests/office_mcp/word/test_document_parser.py`（新建，W1）

### [ ] **Task OT-060** — `tests/office_mcp/word/test_read_word.py`（新建，W1）

### [ ] **Task OT-061** — `tests/office_mcp/word/test_create_word.py` / `test_edit_word.py`（新建，W2）

### [ ] **Task OT-062** — `tests/office_mcp/word/test_merge_word.py` / `test_apply_template_word.py` / `test_edit_word_script.py`（新建，W3）

### [ ] **Task OT-063** — `tests/office_mcp/word/test_legacy_compat.py`（新建，W3）

### [ ] **Task OT-064** — `tests/office_mcp/word/test_e2e_word_tools.py`（新建，W2+）

| 字段 | 内容 |
|------|------|
| **markers** | `@pytest.mark.word` `@pytest.mark.e2e` |

### [ ] **Task OT-065** — `tests/office_mcp/test_office_edit_document.py` 等（改，W3）
...
### [ ] **Task OT-068** — `aiecs/tools/office_tool/registry.py`（新建）
...
### [ ] **Task OT-069** — `aiecs/mcp/office_tool_adapter.py`（改）
...
### [ ] **Task OT-070** — `aiecs/tools/office_tool/gateway/execute_builder.py`（新建）
...
### [ ] **Task OT-071** — `aiecs/tools/office_tool/gateway/call_api.py`（新建）
...
### [ ] **Task OT-072** — 根 `execute_builder.py` / `call_api.py`（改 shim）

### [ ] **Task OT-073** — `aiecs/tools/office_tool/__init__.py`（改）

| 字段 | 内容 |
|------|------|
| **必须完成** | 导出策略：保留 legacy 六工具 re-export 或文档化 deprecated |

### [ ] **Task OT-074** — `tests/office_mcp/test_registry.py`（新建）
...
### [ ] **Task OT-075** — `tests/office_mcp/test_office_tool_adapter.py`（改）
...
### [ ] **Task OT-076** — `tests/office_mcp/test_integration.py`（改）
...
### [ ] **Task OT-077** — `tests/office_mcp/test_office_*.py` → `tests/office_mcp/word/`（搬迁，ADR-023）

| 字段 | 内容 |
|------|------|
| **必须完成** | word 相关 flat 测试迁入 `word/` 或标记 deprecated path |

### [ ] **Task OT-078** — `aiecs/main_mcp.py`（改）
...
### [ ] **Task OT-079** — `docs/LEGACY_TOOL_MIGRATION.md`（完成 OT-012）

### [ ] **Task OT-080** — CHANGELOG（新建或更新）

| 字段 | 内容 |
|------|------|
| **必须完成** | ADR-024 list_tools 变更条目 |

### [ ] **Task OT-081** — Word 工具 `[Word]` description（ADR-025）

| 字段 | 内容 |
|------|------|
| **必须完成** | 6 个 word canonical `TOOL_DEF["description"]` 前缀 |

### [ ] **Task OT-082** — M3 验收 · **Gate G1**
...
### [ ] **Task OT-083** — `presentation/__init__.py` + 目录树（P0）

### [ ] **Task OT-084** — `presentation/parser/txt.py`（P0）

| 字段 | 内容 |
|------|------|
| **必须完成** | 自 `html_parser.parse_txt_*` 迁入（coarse） |

### [ ] **Task OT-085** — `presentation/parser/slides.py`（P0–P1）
...
### [ ] **Task OT-086** — `presentation/schemas/read.py` / `slide_spec.py` / `edit_ops.py`（P1）

### [ ] **Task OT-087** — `presentation/builder/create.py` / `edit.py` / `merge.py` / `template.py`（P1–P2）
...
### [ ] **Task OT-088** — `presentation/tools/read.py`（P0）

| 字段 | 内容 |
|------|------|
| **必须完成** | `office_read_presentation`；`extra={"layouts": ...}` |

### [ ] **Task OT-089** — `presentation/tools/create.py` / `edit.py`（P1）

### [ ] **Task OT-090** — `presentation/tools/merge.py` / `template.py`（P2）

| 字段 | 内容 |
|------|------|
| **工具名** | `office_merge_presentations`, `office_apply_template_presentation` |

### [ ] **Task OT-091** — `registry.py`（改）

| 字段 | 内容 |
|------|------|
| **必须完成** | 追加 presentation 五模块；**M4 后 canonical=13** |

### [ ] **Task OT-092** — `[Presentation]` description 前缀（5 工具）
...
### [ ] **Task OT-093** — `tests/office_mcp/presentation/test_slides_parser.py` 等（全套，§10.1）

### [ ] **Task OT-094** — `tests/office_mcp/presentation/fixtures/layouts_pptx.json` / `layouts_odp.json`（P4，ADR-016）

### [ ] **Task OT-095** — `tests/office_mcp/presentation/test_e2e_presentation_tools.py`

### [ ] **Task OT-096** — legacy pptx txt 回归

| 字段 | 内容 |
|------|------|
| **必须完成** | `office_read_document` pptx 行为不变 |

### [ ] **Task OT-097** — `tests/office_mcp/presentation/test_schemas.py`（P1）

| 字段 | 内容 |
|------|------|
| **必须完成** | layout 枚举校验；非法 slide_index（ADR-016） |

### [ ] **Task OT-098** — `[Presentation]` description 与 registry **M4=13/17** 复核
...
### [ ] **Task OT-100** — `spreadsheet/__init__.py` + 目录树（S0）

### [ ] **Task OT-101** — `spreadsheet/parser/csv.py`（S0）

| 字段 | 内容 |
|------|------|
| **必须完成** | 自 `html_parser.parse_csv_*` 迁入 |

### [ ] **Task OT-102** — `spreadsheet/parser/workbook.py`（S1）

| 字段 | 内容 |
|------|------|
| **必须完成** | sidecar GetSheetsCount + for（ADR-013）；`parse_workbook_json` |

### [ ] **Task OT-103** — `spreadsheet/schemas/read.py` / `workbook_spec.py` / `edit_ops.py`（S2–S3）

| 字段 | 内容 |
|------|------|
| **ADR-015** | A1/range；无 row/col 对外 |

### [ ] **Task OT-104** — `spreadsheet/builder/create.py` / `edit.py` / `merge.py` / `template.py`（S2–S4）

| 字段 | 内容 |
|------|------|
| **ADR-014** | template 显式 `Sheet!A1` + used_range `{{key}}` |

### [ ] **Task OT-105** — `spreadsheet/tools/read.py` / `create.py` / `edit.py` / `merge.py` / `template.py`（S0–S4）

### [ ] **Task OT-106** — `registry.py`（改）：spreadsheet 五模块；**M5 后 canonical=18**（23 在 M6）

### [ ] **Task OT-107** — `[Spreadsheet]` description 前缀（5 工具）
...
### [ ] **Task OT-108** — `tests/office_mcp/spreadsheet/test_workbook_parser.py` 等（全套）

### [ ] **Task OT-109** — `tests/office_mcp/spreadsheet/test_e2e_spreadsheet_tools.py`

### [ ] **Task OT-110** — legacy xlsx csv 回归

### [ ] **Task OT-111** — DS 探针：GetSheetsCount skip（ADR-021，依赖 OT-045b 骨架）

### [ ] **Task OT-112** — M5 E2E 验收 · **Gate G3**
...
### [ ] **Task OT-113** — `pdf/__init__.py` + 目录树（P0）

### [ ] **Task OT-114** — `pdf/parser/pages_txt.py`（P0）

| 字段 | 内容 |
|------|------|
| **必须完成** | ADR-020：`\f` → `--- page N ---` → 单页 + `_note` |

### [ ] **Task OT-115** — `pdf/parser/document.py`（P1）

| 字段 | 内容 |
|------|------|
| **必须完成** | fine read sidecar → pages[] / blocks[] / form_fields[] |

### [ ] **Task OT-116** — `pdf/schemas/read.py` / `page_spec.py` / `edit_ops.py` / `fill_form.py`（P3–P4）

| 字段 | 内容 |
|------|------|
| **ADR-030** | edit_ops **无** `fill_form_field` |

### [ ] **Task OT-117** — `pdf/builder/create.py`（P3）

| 字段 | 内容 |
|------|------|
| **ADR-017** | native / via_docx；**不** auto fallback |

### [ ] **Task OT-118** — `pdf/builder/edit.py`（P3）

### [ ] **Task OT-119** — `pdf/builder/merge.py`（P2）

| 字段 | 内容 |
|------|------|
| **ADR-018** | builder 默认 + `options.engine=conversion` 显式 |

### [ ] **Task OT-120** — `pdf/builder/fill_form.py`（P4）

| 字段 | 内容 |
|------|------|
| **ADR-019** | 逐字段 SetValue |

### [ ] **Task OT-121** — `pdf/tools/read.py` / `create.py` / `edit.py` / `merge.py` / `fill_form.py`（P0–P4）

| 字段 | 内容 |
|------|------|
| **禁止** | `office_apply_template_pdf` |

### [ ] **Task OT-122** — `registry.py`（改）：pdf 五模块；**M6 后 canonical=23**（handlers=27）；确认列表完整

### [ ] **Task OT-123** — `[PDF]` description 前缀（5 工具）
...
### [ ] **Task OT-124** — `tests/office_mcp/pdf/test_pages_txt_parser.py` 等 + `fixtures/acroform_template.pdf`

### [ ] **Task OT-125** — `tests/office_mcp/pdf/test_e2e_pdf_tools.py`

### [ ] **Task OT-126** — M6 E2E 验收 · **Gate G4**
...
### [ ] **Task OT-134** — `tests/office_mcp/gateway/` / `legacy/` 测试子目录（可选）

| 字段 | 内容 |
|------|------|
| **允许** | 自 flat 测试逐步迁入 |

### [x] **Task OT-135** — Shim 删除（**N/A M7**）

| 字段 | 内容 |
|------|------|
| **ADR-022** | M7 **不删** shim；单独 breaking PR |
